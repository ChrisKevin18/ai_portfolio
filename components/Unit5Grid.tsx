...
const filters: { label: string; value: "ALL" | Unit5Pillar }[] = [
  { label: "ALL UNIT-5 TOPICS (09)", value: "ALL" },
  { label: "EPISTEMIC & TRUTH", value: "Epistemic & Truth" },
  { label: "LABOR & CREATIVITY", value: "Labor & Creativity" },
  { label: "WARFARE & GEOPOLITICS", value: "Warfare & Geopolitics" },
  { label: "GLOBAL JUSTICE & GOVERNANCE", value: "Global Justice & Governance" },
];
    ...
    <div className="mx-auto max-w-[1480px]">
      ...
      <div className="mt-12 flex flex-wrap items-center justify-between gap-4 rounded-2xl border border-line bg-card/50 p-4 px-6 text-xs font-mono text-muted backdrop-blur-md">
        <div className="flex items-center gap-2">
          ...
          <span>SHOWING {filteredTopics.length} OF {unit5Topics.length} UNIT-5 TOPICS</span>
        </div>
        <div className="flex items-center gap-3">
          <span className="h-2 w-2 rounded-full bg-green animate-pulse" />
          <span>ALL TOPICS PEER-REVIEWED FOR CIA 3 CRITERIA</span>
        </div>
      </div>
    </div>
  );
